...
urlpatterns=  [
    ...
    path('booking/',include('auth_views.user.conf.views')), # aqui el registro por ruta va ser unico , cada persona se debe de registrar indivualmente
...
urlpatterns=  [
    path('admin/',admin.site.urls), # aqui vamos asignar , el frontend tiene  que asignar el orden de el despligue de la plataforma
    path('email/',include('user_app.urls'),username='users'), # el campo de username va ser el email 
    path('password/',include('user_app.urls'),password='password'),
]
